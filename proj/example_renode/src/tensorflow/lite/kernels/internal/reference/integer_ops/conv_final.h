...
inline void ConvPerChannel(
  ...
  if(input_depth == 12){
    for (int batch = 0; batch < 1 /* batches */; ++batch) {
      for (int out_y = 0; out_y < 24 /* output_height */; ++out_y) {
        const int in_y_origin = out_y;
        for (int out_x = 0; out_x < 24 /* output_width */; ++out_x) {
          const int in_x_origin = out_x;
          for (int out_channel = 0; out_channel < 12 /* output_depth */; ++out_channel) {
            ...
            for (int filter_y = 0; filter_y < 3; ++filter_y) {
              ...
              for (int filter_x = 0; filter_x < 3; ++filter_x) {
                ...
                for (int in_channel = 0; in_channel < 12 /* input_depth */; in_channel += 4) {
  ...
  if(input_depth == 1) {
    for (int batch = 0; batch < 1 /* batches */; ++batch) {
      for (int out_y = 0; out_y < 26 /* output_height */; ++out_y) {
        const int in_y_origin = out_y;
        for (int out_x = 0; out_x < 26 /*output_width */ ; ++out_x) {
          const int in_x_origin = out_x;
          for (int out_channel = 0; out_channel < 12 /* output_depth */; ++out_channel) {
            int32_t acc = cfu_op0(1, 0, 0); // resets acc;
            for (int filter_y = 0; filter_y < 3; ++filter_y) {
